...
import type { Link as ProfileLink } from "./types/type";
...
export default async function PublicProfile({
  ...
  const activeLinks = (user.links || [])
    ...
    .map((link: ProfileLink) => {
      if (link.children && link.children.length > 0) {
        return {
          ...link,
          children: link.children.filter((child: ProfileLink) => {
            if (child.startDate && new Date(child.startDate) > now) return false;
            if (child.endDate && new Date(child.endDate) < now) return false;
            return true;
          }),
        };
      }
  ...
  return (
    <main className={`min-h-screen relative px-4 py-16 theme-${user.theme || "default"}`} style={user.backgroundImage ? { background: 'transparent' } : bgStyle}>
