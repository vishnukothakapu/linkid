        ...
        <header className="sticky top-0 z-50 border-b border-violet-200/60 bg-white/70 shadow-sm shadow-violet-950/5 backdrop-blur-xl dark:border-white/10 dark:bg-zinc-950/70 dark:shadow-black/20">
            <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
                ...
                <nav className="hidden items-center gap-2 rounded-full border border-violet-200/60 bg-white/55 p-1 text-sm text-zinc-600 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.04] dark:text-zinc-400 md:flex">
                    ...
                    className={activeSection === "features" ? "rounded-full bg-violet-600 px-4 py-2 font-medium text-white shadow-sm shadow-violet-500/25" : "rounded-full px-4 py-2 transition-colors hover:bg-violet-50 hover:text-violet-700 dark:hover:bg-white/10 dark:hover:text-violet-200"}>
                        Features
                    </Link>
                    <Link href="/#how" 
                    aria-current={activeSection === "how" ? "location" : undefined}
                    className={activeSection === "how" ? "rounded-full bg-violet-600 px-4 py-2 font-medium text-white shadow-sm shadow-violet-500/25" : "rounded-full px-4 py-2 transition-colors hover:bg-violet-50 hover:text-violet-700 dark:hover:bg-white/10 dark:hover:text-violet-200"}>
                        How it works
                    </Link>
                    <Link href="/#demo" 
                    aria-current={activeSection === "demo" ? "location" : undefined}
                    className={activeSection === "demo" ? "rounded-full bg-violet-600 px-4 py-2 font-medium text-white shadow-sm shadow-violet-500/25" : "rounded-full px-4 py-2 transition-colors hover:bg-violet-50 hover:text-violet-700 dark:hover:bg-white/10 dark:hover:text-violet-200"}>
                        Demo
                    </Link>
                </nav>
